...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (name, email, password, profileImgUrl) VALUES (?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getName(), postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getProfileImgUrl()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int modifyUserPassword(PatchUserPasswordReq patchUserPasswordReq) {
        String modifyUserPasswordQuery = "update User set password = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 password으로 변경한다.
        Object[] modifyUserPasswordParams = new Object[]{patchUserPasswordReq.getNewPassword(), patchUserPasswordReq.getUserIdx()}; // 주입될 값들(nickname, userIdx) 순

        return this.jdbcTemplate.update(modifyUserPasswordQuery, modifyUserPasswordParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }

    public int modifyUserStatus(PatchUserStatusReq patchUserStatusReq) {
        String modifyUserStatusQuery = "update User set status = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 password으로 변경한다.
    ...
    public User getPwdByEmail(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, name, email, password, profileImgUrl, mailFlag, smsFlag, status from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public User getPwdByUserIdx(PatchUserPasswordReq patchUserPasswordReq) {
        String getPwdQuery = "select userIdx, name, email, password, profileImgUrl, mailFlag, smsFlag, status from User where userIdx = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
        int getPwdParams = patchUserPasswordReq.getUserIdx(); // 주입될 email값을 클라이언트의 요청에서 주어진 정보를 통해 가져온다.
    ...
    public List<GetUserRes> getUsersByNickname(String nickname) {
        String getUsersByNicknameQuery = "select * from User where name =?"; // 해당 name을 만족하는 유저를 조회하는 쿼리문
